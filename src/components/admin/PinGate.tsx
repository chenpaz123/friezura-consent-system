...
import { PIN_LENGTH } from "@/lib/pins";

export interface PinGateSuccess {
  /** The digits the user typed — forward this on if a later action needs to re-verify (e.g. deleteConsent). */
  pin: string;
...
interface PinGateProps {
  title?: string;
  cancelLabel?: string;
  onSuccess: (result: PinGateSuccess) => void;
  onCancel: () => void;
  /**
   * "modal" overlays existing content on a dark backdrop (Manual Entry exit,
   * delete re-confirmation). "screen" is a standalone, full-page pink lock
   * screen with the Friezura logo (used to gate the whole /admin dashboard).
   */
  variant?: "modal" | "screen";
}

/**
 * Mobile-friendly, RTL PIN pad. Reused to gate the entire admin dashboard,
 * to gate leaving "Manual Entry" mode, and to re-confirm identity before a
 * super-admin delete. Verification happens server-side via the verifyPin
 * Server Action — this component never knows what the correct PIN(s) are,
 * only pass/fail and which tier matched.
 *
...
export function PinGate({
  ...
  const digitButtonClass =
    "rounded-xl bg-pink-50 py-3 text-lg font-semibold text-slate-700 active:bg-pink-100 disabled:opacity-50";
  const actionButtonClass =
    "rounded-xl py-3 text-sm font-medium text-slate-400 active:bg-slate-100 disabled:opacity-50";
  ...
  const card = (
    <div className="w-full max-w-xs rounded-2xl bg-white p-6 text-center shadow-xl">
      ...
      <div className="mt-3 grid grid-cols-3 gap-3">
        <button type="button" onClick={onCancel} disabled={verifying} className={actionButtonClass}>
          {cancelLabel}
        </button>
        <button type="button" onClick={() => press("0")} disabled={verifying} className={digitButtonClass}>
          0
        </button>
        <button
          type="button"
